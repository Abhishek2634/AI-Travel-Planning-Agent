...
async def run_mcp_travel_planner(source: str, destination: str, start_date_str: str, end_date_str: str,
                                   ...
                                   google_client_secret: str, google_refresh_token: str, tavily_key: str):
    ...
    try:
        ...
        travel_planner = Agent(
            ...
            tools=[mcp_tools, TavilyTools(api_key=tavily_key)] if tavily_key else [mcp_tools],
...
def run_travel_planner(source: str, destination: str, start_date_str: str, end_date_str: str,
                       ...
                       google_refresh_token: str, tavily_key: str):
    """Synchronous wrapper."""
    return asyncio.run(run_mcp_travel_planner(source, destination, start_date_str, end_date_str,
                                               ...
                                               google_refresh_token, tavily_key))
...
with st.sidebar:
    ...
    tavily_key = st.text_input("Tavily API Key (for web search)", value=tavily_key, type="password", 
                                help="Get free API key at https://tavily.com")
    
    # Optional Keys
    st.subheader("Optional Keys")
    accuweather_key = st.text_input("AccuWeather API Key", value=accuweather_key, type="password")
    
    with st.expander("Google Calendar Integration (Optional)"):
    ...
    # Check required keys
    required_keys = google_maps_key and openai_key and tavily_key
    ...
    if required_keys and google_client_id and google_client_secret and google_refresh_token and accuweather_key:
        ...
        st.warning("⚠️ Please enter all required API keys (OpenAI, Google Maps, Tavily)")
...
if st.button("Plan My Trip", type="primary"):
    if not destination:
    ...
    elif not (google_maps_key and openai_key and tavily_key):
        st.error("Please enter all required API keys (OpenAI, Google Maps, and Tavily)")
        ...
        with st.spinner("Creating your travel plan..."):
            try:
                ...
                st.session_state.trip_start_date = start_date_input
